/* FishkeepingLifeCo — Core app (v1.0.2)
   - Bars animate from 0 smoothly
   - Bioload baseline capacity = gallons × 0.7 (strict)
   - Environmental: find overlap/compromise; mark outliers
   - Aggression: rule-driven; Betta with any fin-nipper or Tiger Barb now maxes to 100%
   - Warnings grouped in panels
   - Additional spacing for quantity & Rec Min inputs above Add/Clear
*/
...
function renderTable() {
  ...
  for (const entry of stock) {
    ...
    minus.className='mini'; minus.type='button'; minus.textContent='−';
    minus.addEventListener('click', () => inc(entry.id, -1));
    const plus  = document.createElement('button');
    plus.className='mini'; plus.type='button'; plus.textContent='+';
    plus.addEventListener('click', () => inc(entry.id, +1));
    const del   = document.createElement('button');
    del.className='mini'; del.type='button'; del.textContent='Remove';
...
function renderMetricsAndWarnings() {
  ...
  let tempSetpoint = null; let tempCompromise=false;
  ...
  let phBand = null; let phCompromise=false;
  ...
  if (entries.length && tempSetpoint!=null){
    for (const e of entries){
      ...
      if (tDist>2) bubble(warnEnv,`${e.s.name}: temperature outlier by ~${Math.round(tDist)}°F — not recommended for this stock.`,'bad');
    }
  }
  ...
  // new severe: Betta with any fin-nippers or Tiger Barb
