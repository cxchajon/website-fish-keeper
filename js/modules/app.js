...
document.addEventListener('DOMContentLoaded', () => {
  ...
  qtyEl.addEventListener('input', ()=>{ /* reserved */ });
...
function renderMetricsAndWarnings() {
  ...
  if (entries.length && phBand != null){
    for (const e of entries){
      ...
      if (gap >= 3.0){
        bubble(warnEnv, `${e.s.name}: pH not compatible (≥3.0 away from safe band).`, 'bad');
      } else if (gap > 2.5){
        bubble(warnEnv, `${e.s.name}: pH far from safe band (>2.5).`, 'warn');
      }
    }
  }
