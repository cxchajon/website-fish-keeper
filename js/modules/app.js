...
function boot(){
  ...
  // Add button
  if (els.addFish){
    els.addFish.addEventListener('click', ()=> window.addSelectedFish());
  }
...
window.addSelectedFish = function addSelectedFish(){
  ...
  const hasUser  = qtyEl && String(qtyEl.value || '').trim().length > 0;
  const qty      = hasUser
    ? safeQty(qtyEl.value)
    : safeQty(recMinEl ? recMinEl.value : '1');
