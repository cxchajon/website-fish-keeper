...
| `/cycling-coach/` | ✅ Loader once in `<head>` after consent script. | ✅ Top `ad-slot` placeholder reserved for `8136808291`; bottom `ad-slot` reserved for `5754828160`. | ✅ Banner hooks identical; consent choice updates DOM & GTM state. | ✅ Same `.ttg-adunit` reserve ensures no CLS; placements away from form buttons. | ✅ Legal content separate; footer updated. | **READY** |
