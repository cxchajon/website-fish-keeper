...
| `blackwater` | Canonical vocabulary (validator, conditions rule and environment card all use it; the old `required` / `recommended` spellings are invalid). Engine meaning: `requires` → red warning when tannins are off; `prefers` → amber; `neutral` → assessed as having no tannin preference. `null` → **not assessed** (no cited source makes a tannin/habitat claim); the engine applies no blackwater rule. "Does not require tannins" is not evidence for `neutral`. |
