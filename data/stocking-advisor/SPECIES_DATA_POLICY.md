...
| `behavior.socialMinimum` | A documented conspecific minimum for a species that is **not** schooling (e.g. honey gourami: “not a schooling fish … buy no fewer than 4–6”). The lowest directly supported number is used. The engine represents it as a `social` group (“Not a schooling fish, but keep N+ together”), never as a shoal. `schoolingMinimum` takes precedence when both are set. |
| `blackwater` | Canonical vocabulary (validator, conditions rule and environment card all use it; the old `required` / `recommended` spellings are invalid). Engine meaning (WATER_MODEL.md §4): `requires` → amber while the user's tannins are unknown, red when the user says tannins are off; `prefers` → an informational tip only, never a warning; `neutral` → assessed as having no tannin preference. Use `requires` only when sources establish a captive requirement, not just a blackwater habitat (Rummynose: habitat and biotope advice, but does well in a standard planted aquarium → `prefers`). No selectable species currently has `requires`. `null` → **not assessed** (no cited source makes a tannin/habitat claim); the engine applies no blackwater rule. "Does not require tannins" is not evidence for `neutral`. |
...
Fish-on-fish predation (`fish_risk`, `preys_on_species`) uses the same evidence rule: a prey entry in
`behavior.predationRisks` that names a selectable fish by its exact common name (singular or plural),
e.g. “Small fish (e.g., Neon Tetras)” → Neon Tetra. There is no size-based inference. When predator and
prey are planned together the advisor shows a red compatibility warning.
