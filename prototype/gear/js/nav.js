(() => {
  ...
  function toCanonicalPath(path) {
    ...
    const map = {
      ...
      '/index': HOME_PATH,
      '/stocking-advisor.html': '/stocking/',
      '/stocking-advisor': '/stocking/',
      '/stocking.html': '/stocking/',
      '/stocking': '/stocking/',
      '/stocking/': '/stocking/',
      '/stocking/index.html': '/stocking/',
  ...
  async function mountNav() {
    ...
    try {
      const response = await fetch(`./includes/nav.html?v=${NAV_VERSION}`, { cache: 'no-store' });
      if (!response.ok) {
