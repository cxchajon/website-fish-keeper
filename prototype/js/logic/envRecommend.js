import { clamp, getBandColor } from '/js/logic/utils.js';
import { formatCareTagLabel } from '/js/logic/careTags.js';
import { formatBioloadPercent } from '/js/bioload.js';
...
function toggleEnvCompact({ env, bioloadPercent, aggressionPercent }) {
  ...
  const hasBioloadMeter = Number.isFinite(bio) && bio > 0;
  const hasAggressionChips = Array.isArray(env?.barNotes?.aggression) && env.barNotes.aggression.length > 0;
  const hasAggressionLabel = typeof env?.aggressionLabel === 'string' && env.aggressionLabel.trim().length > 0;
  const shouldCompact = !(hasEnv || hasBioloadMeter || hasAggressionChips || hasAggressionLabel);
...
function renderBars(root, env, { isMobile = false, isEmpty = false } = {}) {
  ...
  const detailChips = isEmpty ? '' : renderChips(env.detailChips ?? []);
  ...
  // UI cleanup — Aggression meter removed (logic retained for badges/warnings) — Oct 2025
  const compatibilityChips = isEmpty ? '' : renderChips(env.barNotes?.aggression ?? []);
  const shouldShowFallback = !isEmpty
    && !compatibilityChips
    && typeof env?.aggressionSeverity === 'string'
    && env.aggressionSeverity !== 'ok'
    && typeof env?.aggressionLabel === 'string'
    && env.aggressionLabel.trim().length > 0;
  const compatibilityFallback = shouldShowFallback
    ? `<span class="env-compat-note" data-role="aggression-label">${escapeHtml(env.aggressionLabel)}</span>`
    : '';
  const compatibilityBlock = compatibilityChips || compatibilityFallback
    ? `<div class="env-compatibility" data-role="compatibility-notes">${compatibilityChips || compatibilityFallback}</div>`
    : '';
  const detailBlock = detailChips ? `<div class="env-detail-chips" data-role="detail-chips">${detailChips}</div>` : '';
